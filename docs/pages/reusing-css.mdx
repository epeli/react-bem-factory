...
sent over the network when server rendering.
...
2. Because BEMed creates these styles it can skip injecting them when doing
   critical CSS server-side rendering if they are not used in the initial render
